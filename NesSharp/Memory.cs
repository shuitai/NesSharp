...
public abstract class Memory
{
    ...
    public abstract byte Read(Address address);
    ...
    public abstract void Write(Address address, byte data);
    ...
    public void ReadBuf(byte[] buffer, Address address, Address size)
    {
        for (int bytesRead = 0; bytesRead < size; bytesRead++)
        {
            Address ReadAddr = (Address)(address + bytesRead);
    ...
    public void ReadBufWrapping(byte[] buffer, int startIndex, Address startAddress, int size)
    {
    ...
    public Address Read16(ushort address)
    {
        byte lo = Read(address);
        byte hi = Read((Address)(address + 1));
    ...
    /// <returns>A 16 bit value representing the two bytes read in a little endian fashion</returns>
    /// <param name="address">The address to read two bytes from</param>
    public Address Read16WrapPage(ushort address)
    {
        Address data;
        if ((address & 0xFF) == 0xFF)
        {
            byte lo = Read(address);
            byte hi = Read((Address)(address & (~0xFF))); // Wrap around to start of page eg. 0x02FF becomes 0x0200
            data = (Address)((hi << 8) | lo);
        }
        else
        {
            data = Read16(address);
